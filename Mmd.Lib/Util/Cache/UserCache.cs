    ...
    public class UserCache : LRUCache<UserData>
    {
       ...
       public UserData FindByUserName( string username )
            ...
            : base( 10000, TimeSpan.FromMinutes( 1 ), TimeSpan.FromHours( 1 ), null )
        {
            ...
            _findByUserID = AddIndex<int>( "UserID", user => user.UserID, LoadFromUserID );
            _findByUserName = AddIndex<string>( "UserName", user => user.UserName, LoadFromUserName );
            IsDataValid();
        }
        ...
        private DataType GetDataRow2<DataType>(string sql, object arg1, Func<IDataRecord,DataType> loadFunc) where DataType : class
        {
            using (SqlConnection conn = new SqlConnection(""))
            {
                conn.Open();
        ...
        private bool IsDataValid()
        {
            ...
            _tableVersion = (long?)GetDataRow<object>( sql, 1, dr => dr.GetInt64( 1 ) ) ?? 0;
        ...
        private UserData LoadFromUserID( int userid )
        {
            ...
            return GetDataRow<UserData>( sql, userid, dr => new UserData { 
                UserID = userid, 
        ...
        private UserData LoadFromUserName( string username )
        {
            ...
            return GetDataRow<UserData>( sql, username, dr => new UserData { 
